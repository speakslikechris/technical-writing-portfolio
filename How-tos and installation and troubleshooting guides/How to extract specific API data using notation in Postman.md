...
I use Postman for API documentations. So, testing APIs and documenting them. I don't have a highly technical background, even though I'm familiar enough with languages such as XML and JSON to read and manipulate them.
However, I ran into an issue with Postman, slowing down my API testing.

## The problem
Postman is great for testing APIs using all of the common and uncommon methods (GET, PUT, DELETE, PATCH, etc). However, I couldn't directly fetch specific responses using the dot notation like I would with a command line interface, for example.
This isn't a data filteration problem. For that, you could just set parameters in Postman. Instead, I wanted to retrieve specific API data such that responses wouldn't output the entire JSON text, but a specific data type.
...
```
In this case, I'd like to document how developers can get a certain parameter, such as the windspeed or weather description, printed or displayed in their applications.
In cURL, you can easily achieve this using dot notation. But what if you wanted to do the same thing within Postman?
...
4. Set a constant to represent your JSON response. The following sample code shows how I approach this. You can modify `responseData` to be anything you want.
`const responseData=pm.response.json();`
5. Set up a constant to define the data returned in the output. The following sample code shows how I approach this. You can modify `content` to be anything you want.
...
`console.log("Main Weather: " + content);`
>[!NOTE]
...
8. The console now outputs the result you called for.

Et voila!

## Objects vs. arrays
There's a difference in how you call on items in an object or array. In the example above, the printed item is in an array. If you want to call on an item in an object instead, the notation to use is much simpler.
